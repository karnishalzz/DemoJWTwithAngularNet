...
export class DashboardComponent implements OnInit {
    ...
    setExamPage(pageNumber) {
        this.examPage.pageNumber = pageNumber;
        this.getExamList();
    }

    getExamList() {
        this._service.get('evaluation-exam/trainee-wise-exam', this.examPage).subscribe(res => {
            if (res.Status === ResponseStatus.Warning) {
              this.toastr.warning(res.Message, 'Warning!', { timeOut: 2000 });
              return;
            }
            ...
            this.openExamList = res.Data.Records;
            this.examPage.totalElements = res.Data.Total;
            this.examPage.totalPages = Math.ceil(this.examPage.totalElements / this.examPage.size);
            setTimeout(() => {
